...
class RPCServer:
    ...
    async def handle(self, raw: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            response = RPCResponse(id=request.id, error=RPCError(code=-32603, message=str(exc)))
    ...
    async def _dispatch_storage(self, path: list[str], params: dict[str, Any]) -> Any:
        ...
        if method == 'get_branch':
            ...
            if not isinstance(item_id, int):
                raise RPCError(code=-32602, message='get_branch.last_item_id must be an integer')
    ...
    async def _dispatch_tools(self, path: list[str], params: dict[str, Any]) -> Any:
        ...
        if method == 'search':
            ...
            return runtime.searcher.search(params['query'], limit=params.get('limit', 5))

        if method == 'schemas':
            return self._ctx.run.agent.tool_runtime.schemas()
        if method == 'resolve':
            descriptor = self._ctx.run.agent.tool_runtime.resolve(params['name'])
            if descriptor is None:
                return None
            return {
                'id': descriptor.id, 'namespace': descriptor.namespace,
                'alias': descriptor.alias, 'name': descriptor.name, 'doc': descriptor.doc,
                'schema': descriptor.schema,
            }
        if method == 'aliases':
            return list(self._ctx.run.agent.tool_runtime.modules.keys())
        if method == 'list':
            alias = params['alias']
            return [
                {
                    'id': descriptor.id, 'name': descriptor.name, 'doc': descriptor.doc,
                    'schema': descriptor.schema, 'metadata': descriptor.metadata,
                }
                for descriptor in self._ctx.run.agent.tool_runtime.modules.get(alias, [])
            ]
...
def _call_arguments(params: dict[str, Any]) -> tuple[list[Any], dict[str, Any]]:
    if 'args' in params or 'kwargs' in params:
        ...
        return list(args) if isinstance(args, list) else [], dict(kwargs) if isinstance(kwargs, dict) else {}
    return [], dict(params)
...
def _parse_origin(data: dict[str, Any]):
    from ....api.dialog import DialogOrigin, ORIGIN_REGISTRY
    platform = data.get('platform')
    for origin_cls in ORIGIN_REGISTRY.values():
        platform_field = origin_cls.model_fields.get('platform')
        if platform_field is not None and platform_field.default == platform:
            return origin_cls.model_validate(data)
    return DialogOrigin.model_validate(data)
